...
def plot_data(data, indicators, syn_axis=None):
    ...
    p.xaxis.major.label_orientation = math.pi / 4
...
def on_button_click(ticker1, ticker2, start, end, indicators):
    ...
    p2 = plot_data(df2, indicatiors, sync_axis=p1.x_range)
    curdoc().clear()
...
date_picker_from = DatePicker(title="Start Date", value="2020-01-01", min_date="2000-01-01", max_date=dt.datetime.now().strtime("%Y-%M-%D"))
date_picker_to = DatePicker(title="Start Date", value="2020-02-01", min_date="2000-01-01", max_date=dt.datetime.now().strtime("%Y-%M-%D"))

indicator_choice = MultiChoice(opitons=["100 Day SMA", "30 Day SMA", "Linear Regression Line"])

load_button = Button(lable="Load Data",button_type="success")
